sim.agent<-function(cfg){
  ...
  #set parameters
  alpha = cfg$alpha
  omega  = cfg$omega #this defines whether non-punishment reinforcing will occur.
  ...
  for (subject in 1:Nsubjects){
    ...
    for (trial in 1:Ntrials){
      ...
      if (trial%%100==0){ #There is a trigger every once in a while
        #anxiety[trial+1]=sample(c(25,50,100),1) #Anxious
        anxiety[trial+1]=sample(c(10,20,40),1) #Calm
        }
      ...
      objective = outcome
      PE = compare - values[state,action]
