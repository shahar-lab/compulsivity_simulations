sim.agent <- function(subject, cfg) {
  ...
  betas[,10]=cfg$beta_wait[subject] #different self control for waiting action
  ...
  pr        = matrix(rep(cfg$pr[subject]),Nstates,Nactions)        #perseveration cost
  ...
  exposure_intensity=cfg$exposure_intensity
  treatment_cost=cfg$treatment_cost
  ...
  for (period in 1:Nperiods){
    ...
    if(treatment[period]=="during"){
      trigger_frequency=trigger_frequency/exposure_intensity #making triggers more often during treatment
      pr[1,1:9]=pr[1,1:9]*treatment_cost #enhancing cost for all actions in the dangerous state but not for "waiting"
    }
    
    else if(treatment[period]=="after"){
      trigger_frequency=trigger_frequency*exposure_intensity #making triggers less often after treatment
      pr[1,1:9]=pr[1,1:9]/treatment_cost #reducing cost for all actions but not for "waiting" in the dangerous state
    }
